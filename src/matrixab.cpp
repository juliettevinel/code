...
double sthcell_a (double** phi, cell pt, grid cells) {  //   producing the coefficient corresponding to the south cell

	return  valbot(phi, pt,cells) /2. /cells.rj[pt.j] /(cells.rj[pt.j] + cells.rj[pt.j-1]) ;

}  //endof function botbound_a


double nthcell_a (double** phi, cell pt, grid cells) {  // producing the coefficient corresponding to the north (top) cell

	return  valtop(phi, pt,cells) /2. /cells.rj[pt.j] /(cells.rj[pt.j] + cells.rj[pt.j+1]) ;

}


double wstcell_a (double** phi, cell pt, grid cells)  {		// producing the coefficient corresponding to the west (left)  cell

  return  valback(phi, pt,cells) /2. /cells.ri[pt.i] /(cells.ri[pt.i] + cells.ri[pt.i-1]) ;

}

double estcell_a (double** phi, cell pt, grid cells) {		// producing the coefficient corresponding to the east (right)  cell

  return  valfor(phi, pt,cells) /2. /cells.ri[pt.i] /(cells.ri[pt.i] + cells.ri[pt.i+1]) ;
