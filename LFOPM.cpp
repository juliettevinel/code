...
// same logic as in the previous use of <<  and  >> 
	...
	double **rho;
	rho = matrix_np(n+2,m+2);
	//double **rho1;
	//rho1 = matrix_np(n+2,m+2);
	//double **rhonp1;
	//rhonp1 = matrix_np(n+2,m+2) ;
	double **rho_s;
	rho_s = matrix_np(n+2,m+2);

	double **temp ;
	temp = matrix_np(n+2,m+2);
	//double **temp1 ;
	//temp1 = matrix_np(n+2,m+2);

	initialize(temp, 0, n+1, m+1) ;

	double **temp_s ;
	temp_s = matrix_np(n+2,m+2);
 ...
 ******************************************/
    ...
    // valeur initiales pour rho et temp vu qu'on les considere constants
	...
	imp_bound_Y(Y1, 1., Yval_t, Yval_b, n,m) ;
	...
	//replace(rhonp1, rho1, 0, n+1, m+1) ;
	  ...
	  for (int i=1 ; i<=n ; i++)
	    for (int j=1 ; j<=m ; j++)
	      M_zero += rho[i][j] * phin[i][j] * 4. *cells.ri[i] *cells.rj[j] ;
	

	 M_re_compu(M_re, temp, phi_sn, n,m) ;




	renew_mu_k(mu, kf, temp, 0, n+1, m+1) ;
...
*/
	...
	*/
	...
	calc_Uc (phiUc, M_re, rho, rho, phinp1, phin, u_infl, n,m, cells, dt) ;
	...
	if (delta_rank == 'c') {
		filldelta(delta_x,1,n,m,u1,phin, rho,mu);
		filldelta(delta_y,2,n,m,v1,phin, rho,mu);
	}
	else if (delta_rank == 's') {
	  filldelta_s(delta_x,u1,v1,rho_s,mu,phin,n,m);
	  filldelta_s(delta_y,u1,v1,rho_s,mu,phin,n,m);
	}




	fluxinit(f1, f2, u1, v1, rho, phin, n, m, cells) ;

	rescompu(resu1,resv1, u1,v1, f1,f2, phin, rho, mu, F, n,m,cells) ;

	replace(resu0, resu1, 1, n,m ) ;
	replace(resv0, resv1, 1, n,m ) ;

	h_compu(h,u1, v1, rho, mu, kf, phin, n,m );

	res_Y_compu(res_Y1, f1,f2, Y1, rho, phin,D,R, cells, n,m) ;
	...
	imp_bound_p(p2D , rho, n , m , y_val, cells) ;
	...
	// Therefore, only the utilde_i is of interest
	  calc_utilde_pred(utilde_i,utilde_j, u1,v1, resu1,resv1,resu0,resv0, p2D, rho_s,rho,mu,phist, phin, delta_x, delta_y, y_val, cells,n,m,dt, flag ) ;
	  // boundary conditions for utilde
	  imp_bound_u_tild(Bcomb_ut, Bcomb_ub, p2D, rho, u_infl, utilde_i, n,m, dimy, cells, dt) ;
 ...
 ******************************************/
	...
	for (int ti = 1 + step_init ; ti <= tsteps ; ti++) {  // ti for t_i, integer number of step
	  ...
	  if (t<4.)
		...
		//calculate T*, T_s*
	  replace(Y0, Y1) ;
	  calc_Y_pred(Y1,Y0,rho,rho_s, res_Y1,res_Y0, p0,M_zero,D, phin, n,m, pmedia_jmax, cells, dt);
	...
	//  replace(temp_s0,temp_s1) ;
	 ...
	 // ks_compu(ks_x, ks_y, temp_s1, n,m) ;
	  ...
	  calc_utilde_pred(utilde_i,utilde_j, u1,v1, resu1,resv1,resu0,resv0, p2D, rho_s,rho,mu,phist, phin, delta_x, delta_y, y_val, cells,n,m,dt, flag ) ;
	  // boundary conditions for utilde
	  imp_bound_u_tild_conv(Bcomb_ut, Bcomb_ub, p2D, rho, u_infl, utilde_i, utilde_i0, Uc, n,m, dimy, cells, dt) ;
	  ...
	  calc_p (p2D,p, matA,matA_pos,precond,matB, utilde_i, utilde_j, rho,rho, rho_s, phin, phinm1, phist, M_re, y_val, cells, n,m,dt, flag, ti) ;
	  // boundary conditions for pressure
	  imp_bound_p(p2D ,rho_s, n , m , y_val, cells) ;
	  ...
	  */
	  ...
	  calc_vel (u1,v1, utilde_i, utilde_j, p2D, rho_s, n,m, y_val, cells, dt) ;
	  //impose boundary conditions to u*

	  if (Bcomb_vel == 2)
	    calc_Uc (phiUc, M_re, rho_s, rho, phist, phin, u_infl, n,m, cells, dt) ;
	  ...
	  fluxcompu(f1, f2, utilde_i, utilde_j, rho_s, phist, p2D, y_val, cells, n, m, dt) ;


	  if (delta_rank == 'c') {
	    filldelta(delta_x,1,n,m,u1,phist, rho_s,mu);
	    filldelta(delta_y,2,n,m,v1,phist, rho_s,mu);
	  }
	  else if (delta_rank == 's') {
	    filldelta_s(delta_x,u1,v1,rho_s,mu,phist,n,m);
	    filldelta_s(delta_y,u1,v1,rho_s,mu,phist,n,m);
	  }
	  ...
	  rescompu(resu1, resv1, u1,v1, f1,f2, phist, rho_s, mu, F, n,m,cells) ;

	  h_compu(h,u1, v1, rho_s, mu, kf, phist, n,m );

	  M_re_compu(M_re, temp, phi_sst, n,m) ;

	  // the same for res_t
	  replace(res_Y0, res_Y1) ;

	  res_Y_compu(res_Y1, f1,f2, Y1, rho_s, phist, D,R, cells, n,m) ;
	...
	//  replace(res_ts0, res_ts1) ;
	  ...
	  */
	  ...
	  //temp0 -> (n)
	  //temp1 -> (n+1)
	  calc_Y_corr(Y1,Y0, rho, rho_s, res_Y1,res_Y0, p0,M_zero,D, phinp1, phist, phin, n,m, pmedia_jmax, cells, dt);
	 ...
	 // ks_compu(ks_x, ks_y, temp_s1, n,m) ;
	  ...
	  // ATTENTION : UTILDE CALCULATION CONFIGURED FOR WALL ON TOP
	  calc_utilde_corr(utilde_i,utilde_j, u0,v0, resu1,resv1,resu0,resv0, p2D, rho,rho,mu,phinp1, phin, delta_x, delta_y, y_val, cells,n,m,dt, flag ) ;
	  // boundary conditions for utilde
	  imp_bound_u_tild_conv(Bcomb_ut, Bcomb_ub, p2D, rho, u_infl, utilde_i, utilde_i0, Uc, n,m, dimy, cells, dt) ;
	  imp_bound_v_tild_conv(Bcomb_vt, Bcomb_vb, v_infl, utilde_j, utilde_j0, Uc, n,m, cells, dt) ;
	  ...
	  calc_p (p2D,p, matA,matA_pos,precond,matB, utilde_i, utilde_j, rho,rho, rho, phin, phinm1, phinp1, M_re, y_val, cells, n,m,dt, flag, ti) ;
	  // boundary conditions for pressure
	  imp_bound_p(p2D ,rho, n , m , y_val, cells) ;
	  ...
	  calc_vel (u1,v1, utilde_i, utilde_j, p2D, rho, n,m, y_val, cells, dt) ;
	  ...
	  fluxcompu(f1, f2, utilde_i, utilde_j, rho, phinp1, p2D, y_val, cells, n, m, dt) ;


	  if (delta_rank == 'c') {
	    filldelta(delta_x,1,n,m,u1,phinp1, rho,mu);
	    filldelta(delta_y,2,n,m,v1,phinp1, rho,mu);
	  }
	  else if (delta_rank == 's') {
	    filldelta_s(delta_x,u1,v1,rho,mu,phinp1,n,m);
	    filldelta_s(delta_y,u1,v1,rho,mu,phinp1,n,m);
	  }
	  ...
	  rescompu(resu1, resv1, u1,v1, f1,f2, phinp1, rho, mu, F, n,m,cells) ;
	  ...
	  res_Y_compu(res_Y1, f1,f2, Y1, rho ,phinp1, cp, D,R, cells, n,m) ;

	  //res_ts_compu(res_ts1, temp_s1, temp1, phi_snp1, ks_x, M_re, h, cells, pmedia_imin, pmedia_imax, pmedia_jmin , pmedia_jmax ) ;


	  calc_Y_corr(Y1,Y0, rhonp1, rho, res_Y1,res_Y0, p0,M_zero, D, phinp1, phinp1, phinp1, n,m, pmedia_jmax, cells, dt);
	  ...
	  calc_mass_eq (mass_balance, rho, rho, phinp1, phin, u1, v1, M_re, n,m, cells, dt) ;


	  h_compu(h,u1, v1, rho, mu, kf, phinp1, n,m );

	  M_re_compu(M_re, temp, phi_snp1, n,m) ;


	  // Same as the comment above for res_T and res_Ts
	  // replace(res_t0,res_t1,1, n,m);

	  res_Y_compu(res_Y1, f1,f2, Y1, rho,phinp1, D,R, cells, n,m) ;
